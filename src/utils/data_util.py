...
def get_coco_dataset(name, image_set, is_train, data_path):
    transforms = [ToTensor()]
    if is_train:
    ...
    ds = ds_fn(p, image_set=image_set, transforms=Compose(transforms))
...
def get_coco_data_loaders(dataset_config, batch_size, distributed):
    ...
    train_dataset, num_classes = get_coco_dataset(dataset_name, dataset_splits['train'], True, root_dir_path)
    val_dataset, _ = get_coco_dataset(dataset_name, dataset_splits['val'], False, root_dir_path)
    test_dataset, _ = get_coco_dataset(dataset_name, dataset_splits['test'], False, root_dir_path)
    ...
    test_data_loader = torch.utils.data.DataLoader(test_dataset, batch_size=1, sampler=test_sampler,
                                                   num_workers=num_workers, collate_fn=misc_util.collate_fn)
    return num_classes, train_data_loader, val_data_loader, test_data_loader

...
def get_argparser():
    ...
    parser.add_argument('--log', default='./log/', type=str, help='log directory path')
    return parser
...
def setup_model(args):
    ...
    # create model
    if args.pretrained:
        logging.info("=> using pre-trained model '{}'".format(args.model))
...
def get_data_loaders(train_dir_path, valid_dir_path, test_dir_path, args):
    ...
    valid_loader = torch.utils.data.DataLoader(
        datasets.ImageFolder(valid_dir_path, transforms.Compose([
    ...
    test_loader = torch.utils.data.DataLoader(
        datasets.ImageFolder(test_dir_path, transforms.Compose([
...
def compute_topk_precisions(output, target, topks=(1,)):
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        prec1, prec5 = compute_topk_precisions(output, target, topks=[1, 5])
...
def test(data_loader, model, criterion, args, is_valid=False):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
...
def validate(val_loader, model, criterion, args):
    return test(val_loader, model, criterion, args, is_valid=True)
...
def main(args):
    ...
    # Data loading code
    train_dir_path = os.path.join(args.data, 'train')
    valid_dir_path = os.path.join(args.data, 'val')
    test_dir_path = os.path.join(args.data, 'test')
    train_loader, valid_loader, test_loader, train_sampler =\
        get_data_loaders(train_dir_path, valid_dir_path, test_dir_path, args)
    if not args.evaluate:
        build_model(model, train_loader, valid_loader, train_sampler, criterion, optimizer, best_prec1, args)
    test(valid_loader, model, criterion, args)
